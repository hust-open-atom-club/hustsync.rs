//! Smoke test for the contract-test harness (T1).
//!
...
//! (T3, T5, T7, T9) build on these primitives without re-implementing them.
...
async fn smoke_spawn_manager_ping_200() {
    ...
    // The response must be a JSON object — shape compliance is asserted in T3.
    assert!(
        json.is_object(),
...
/// accidentally deleted, giving a clear signal before fixture-dependent T3+
